...
in memory. This eliminates call-linkage overhead and can expose significant optimization
opportunities. Using the "inline" specifier is only a suggestion to the compiler that an inline
expansion can be performed; the compiler is free to ignore the suggestion.
*/

//creates pool entry point with instruction data
#[inline(always)]
...
impl<'a, 'b, P: Pool> PoolProcessor<'a, 'b, P> {
    ...
    //Youtube - process instructions to create pool
    fn process_instruction(&self) -> PoolResult<()> {
        ...
        match (&mut pool_state, &self.request) {
            ...
            //creates basket.  Basket is like an index which holds the assets.  Also has pool mint_tokens
            //which symolize assets  Greats baskets
    ...
    fn initialize_pool(&self, request: &InitializePoolRequest) -> PoolResult<()> {
        ...
        //set states includng the assetws
