...
impl<'ob> OrderBookState<'ob> {
    ...
    //YOUTUBE Find best bid https://www.investopedia.com/terms/n/nbbo.asp
...
impl<'ob> OrderBookState<'ob> {
    fn new_ask(
        ...
        let crossed;
        //loops through processing the bid
        let done = loop {
            //finds best possible bid
            ...
            //if nothing left to trade break out
            if trade_qty == 0 {
                break true;
            }
            ...
            if order_would_self_trade {
                ...
                //process trade if owner trades with self
        ...
        //if unable to finish return data on unfilled qty
        if !done {
